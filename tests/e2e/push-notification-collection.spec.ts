...
test.describe('Push Notification Extension - E2E Tests', () => {
  ...
  test('deve verificar que a coleção "PushNotification" existe no menu', async () => {
    ...
    const hasPushNotification = navText.toLowerCase().includes('pushnotification') || 
                                navText.toLowerCase().includes('push notification') ||
                                navText.toLowerCase().includes('notifica');

    // Screenshot da navegação
    await sharedPage.screenshot({ path: 'tests/e2e/screenshots/navigation-with-push.png', fullPage: false });

    expect(hasPushNotification).toBeTruthy();
  });

  test('deve acessar a coleção PushNotification', async () => {
    // Navegar para a coleção PushNotification
    await sharedPage.goto('/admin/content/PushNotification', { waitUntil: 'networkidle' });
    ...
    await sharedPage.screenshot({ path: 'tests/e2e/screenshots/push-notification-collection.png', fullPage: true });
  ...
  test('deve exibir os campos corretos na coleção PushNotification', async () => {
    ...
    const createButton = await sharedPage.locator(
    ...
    if (isCreateVisible) {
      ...
      await sharedPage.screenshot({ path: 'tests/e2e/screenshots/push-notification-form.png', fullPage: true });
  ...
  test('deve verificar que os endpoints de push notification estão registrados', async () => {
    ...
    const endpointResponse = await sharedPage.context().request.get('/push-notification', {
      headers: {
  ...
  test('deve criar um item de PushNotification via API', async () => {
    ...
    const createResponse = await sharedPage.context().request.post('/items/PushNotification', {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
      data: {
    ...
    expect(createdItem.data.endpoint).toBe('https://fcm.googleapis.com/fcm/send/test-endpoint');

    // Screenshot após criação
    await sharedPage.goto('/admin/content/PushNotification', { waitUntil: 'networkidle' });
    await sharedPage.waitForTimeout(2000);
    await sharedPage.screenshot({ path: 'tests/e2e/screenshots/push-notification-with-item.png', fullPage: true });
  });
  ...
  test('deve verificar que o item criado aparece na listagem', async () => {
    ...
    const hasTable = await sharedPage.locator('table, [role="table"], .v-grid').isVisible({ timeout: 10000 }).catch(() => false);

    if (hasTable) {
      // Verificar se há pelo menos um item na tabela
      const tableContent = await sharedPage.textContent('table, [role="table"], .v-grid');
      ...
      const emptyState = await sharedPage.locator('.empty-state, .v-info').isVisible({ timeout: 5000 }).catch(() => false);
    ...
    // Screenshot final
    await sharedPage.screenshot({ path: 'tests/e2e/screenshots/push-notification-final.png', fullPage: true });
  });
});
