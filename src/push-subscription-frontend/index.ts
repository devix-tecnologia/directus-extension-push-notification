 ...
 *
 * Este hook usa `embed` para injetar código JavaScript que:
 ...
 *
 * Referência: https://github.com/nerkarso/directus-extensions (current-role hook)
 */
...
export default defineHook(({ embed }, { env }) => {
  ...
  // Script minificado será injetado no body do Directus
  const pushNotificationScript = `
<script>
(function() {
  'use strict';
  ...
  const DEBUG = true; // Sempre ativo para debug
  ...
  // Verifica se o usuário está logado e quer push
  ...
  // Registra a subscription no backend
  ...
  async function initPushNotification() {
    ...
    try {
      // Registrar Service Worker no endpoint correto
      ...
      if (subscription) {
        ...
        // Registra novamente para garantir que está no backend (idempotente)
  ...
  // Aguarda o DOM estar pronto e a autenticação do Directus
  ...
  async function main() {
    ...
    // Aguardar autenticação do Directus (max 10 segundos)
    ...
    // Pequeno delay para garantir que o Directus terminou de carregar
    setTimeout(initPushNotification, 1000);
  }

  // Também re-executa quando a URL muda (SPA navigation)
  let lastUrl = location.href;
  new MutationObserver(function() {
    if (location.href !== lastUrl) {
      lastUrl = location.href;
      log('URL mudou para:', location.href);
      // Re-verificar quando navegar (pode ter feito login)
      setTimeout(initPushNotification, 1000);
    }
...
`;
  ...
  embed("body", pushNotificationScript);
});
